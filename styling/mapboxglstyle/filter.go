...
// func (f *ComparativeFilter) IsObjectShown(sourceLayer string, tags []*ownmap.OSMTag, objectFilterType string) bool {
// 	switch f.category {
// 	case FilterThingType:
// 		if len(f.operands) > 1 {
// 			panic("unsupported: handling multiple operands with a $type category")
// 		}
// 		filterType := f.operands[0]
// 		return filterType == objectFilterType
// 	case FilterThingClass:
// 		for _, tag := range tags {

// 		}
// 	default:
// 		panic("unhandled comparative filter operator: " + f.operator)
// 	}

// 	// ["in","class","residential","suburb","neighbourhood"]
// 	switch f.operator {
// 	case FilterOperatorEquals:
// 		return isIn(f.operator, f.operands, sourceLayer, tags, objectFilterType)
...
func (f *ComparativeFilter) GetAllPossibleTagKeys(sourceLayer string) []*ownmapdal.TagKeyWithType {
	...
	// 		tags := mapMapboxGLClassToOSMTags(operand, sourceLayer)
	// 		for _, tag := range tags {
	...
	// 		}
	// 	}
	// case SubClassCategory:
	// 	// mapboxGL subclass
	// 	for _, operand := range f.operands {
	// 		tags := mapMapboxGLSubclassToOSMTags(operand, sourceLayer)
	// 		for _, tag := range tags {
	// 			types := getPossibleObjectTypesForOSMTags(tag)
	// 			for _, objectType := range types {
	// 				tagsWithTypes = append(tagsWithTypes, &ownmapdal.TagKeyWithType{
	...
	// 	}
	// default:
	// 	log.Printf("unimplemented category: %T", f.category)
	// }
...
// func isIn(category, sourceLayer string, tags []*ownmap.OSMTag, objectFilterType string) bool {
// 	switch category {
// 	case FilterThingType:
...
// 		return false
// 	default:
// 		panic(fmt.Sprintf("unknown thing: %q. Sourcelayer: %q\n", thing, sourceLayer))
// 		return false
// 	}
// }

// func isObjectShown(filter *FilterContainer, sourceLayer string, tags []*ownmap.OSMTag, objectFilterType string) bool {
// 	if filter == nil {
// 		return true
// 	}
